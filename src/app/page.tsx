...
export default function HomePage() {
  return (
    <div className="min-h-[calc(100vh-12rem)] flex flex-col items-center justify-center">
      <h1 className="font-serif text-5xl md:text-6xl font-bold text-ink tracking-tight text-center">
        Welcome to PantryChef, R!
      </h1>
      <p className="text-lg text-ink-muted mt-3 text-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-3xl mt-12">
        {cards.map((card) => (
          <Link
            key={card.href}
            href={card.href}
            className={`group block p-8 bg-cream-dark rounded-2xl border border-cream-dark border-t-4 ${card.accent} shadow-sm hover:shadow-md hover:scale-[1.02] transition-all duration-200 text-center`}
          >
